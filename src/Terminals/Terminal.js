...
const create_initial_text = () => {
  let result = [
    <h2>Masihullah Omer</h2>,
    <h3>
      This is a Fully Interactive Terminal Insprired Portfolio Page
      <br />
    </h3>,

    <p>
      To Use the Portfolio Either{" "}
      <u className="attention">Use the Navigation</u>
    </p>,
    <p>
      Or <u className="attention">Explore the Terminal</u>
      <br />
      <br />
    </p>,
  ...
  starter_command_descriptions.forEach((command_obj, index) => {
    result.push(
      <p className="indented">
        <b className="I">[{index + 1}]</b> or{" "}
        <b className="I">[open {command_obj.name}]</b>:{" "}
...
const terminalSubmit = (e, allPackages) => {
  ...
  commands.forEach((indivCommand) => {
    ...
    if (addition === "clear") tempArr = create_initial_text();
    else tempArr = tempArr.concat(addition);
  });
...
const Handle_allPackages = (allPackages) => {
  return useEffect(() => {
    ...
    if (!allPackages.inView) return;
    allPackages.interval.current.id = setInterval(
      allPackages.interval.current.function,
      1000
    );
...
const PackageRefs = (props) => {
  ...
  const interval = useRef({
    ...
    function: () => {
      ...
      else inputRef.current.value = inputRef.current.value + "▮";
