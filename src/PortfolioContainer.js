...
class PortfolioContainer extends PureComponent {
  ...
  addTab = (tabName) => {
    ...
    if (
      tabName !== "Terminal" &&
      tempTabs.findIndex((tab) => tab.name === tabName) !== -1
    ) {
  ...
  removeTab = (id) => {
    ...
    else index = tempTabs.findIndex((tab) => tab.id === id);

    if (index <= 0) return false;
    else tempTabs.splice(index, 1);
  ...
  selectTab = (id) => {
    ...
    for (let i = 0; i < tempArr.length; i++) {
      ...
      } else tempArr[i].displayed = false;
    }
    tempArr.reverse();
  ...
  DisplayCurrentWindow = () => {
    ...
    let result = null;
    if (this.state.tabs[index].name === "About") result = <About></About>;
        ...
        {Footer.map((footerItem, index) => {
          ...
          return (
            <li key={`link-key-${index}`}>
              <a href={footerItem.link} target="_blank" rel="noreferrer">
                <Icon className="svg" title="" />
              </a>
            </li>
          );
        })}
  ...
  RightBar() {
    return (
      <div className="emailLine">
        <div className="email">
          <span onClick={() => this.addTab("Contact")}>MasihullahOmer</span>
        </div>
      </div>
    );
  }
  ...
  render() {
    return (
      <div id="app">
        ...
        <a
          href="https://github.com/OmerMasih"
          target="_blank"
          id="footer"
          rel="noreferrer"
        >
          ©️2023 | Made with 💛 by Masihullah Omer
        </a>
      </div>
    );
  }
