...
const initializeLocalStorage = () => {
  let tree = localStorage.getItem("Masihullah Omer-files");
  if (tree !== null) return;
...
const saveData = (tree) => {
  ...
  localStorage.setItem("Masihullah Omer-files", stringFileStructure);
};
...
export default class OS extends React.Component {
  ...
  terminalString = this.user + "MasihOmer🧑🏻‍💻:~$ ";

  constructor(props) {
    super(props);
    initializeLocalStorage();
    this.tree = JSON.parse(localStorage.getItem("Masihullah Omer-files"));
  ...
  cd(parameters, path) {
    ...
    if (numberOfReversals >= absoluteSystemPath.length) return "";
  ...
  rm(allPackages) {
    ...
    let removedItem = this.currentDirectory.findIndex((el) =>
      el.name.match(fileName)
    );
    while (removedItem >= 0) {
      let permissions = this.currentDirectory[removedItem].owner;
      let permissionsCheck =
        permissions.length === 0 || permissions.includes(this.user);
      ...
      removedItem = this.currentDirectory.findIndex(
        (el) => el.name.match(fileName) && permissionsCheck
      );
    }
  }
  ...
  open(parameters) {
    ...
    if (index >= 0) return this.currentDirectory[index].owner;
    else return false;
  }

  su(user) {
    this.terminalString = this.terminalString.substring(
      this.terminalString.indexOf("@")
    );
